...
class BridgeServiceTest {
    ...
    void setup() {
        ...
        doReturn([THRIFT_SIMPLE_FIELD, thriftTest()] as Object[]).when(thriftConverter).parseArgs(restRequest())
    }

    @Test
    void "should proxy request to thrift"() {
        doReturn(thriftResponse()).when(thriftClient).testOperation(THRIFT_SIMPLE_FIELD, thriftTest())
    ...
    void "should proxy exception from thrift"() {
        doThrow(thriftException()).when(thriftClient).testOperation(THRIFT_SIMPLE_FIELD, thriftTest())
